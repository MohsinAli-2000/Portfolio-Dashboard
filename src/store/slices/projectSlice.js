...
export const getAllProjects = () => async (dispatch) => {
  ...
  try {
    const response = await axios.get(
      `${APIBaseURL}/api/v1/projects/get-all-projects`,
...
export const addNewProject = (data) => async (dispatch) => {
  ...
  try {
    const response = await axios.post(
      `${APIBaseURL}/api/v1/projects/add-project`,
      data,
      {
        withCredentials: true,
...
export const deleteProject = (id) => async (dispatch) => {
  ...
  try {
    const response = await axios.delete(
      `${APIBaseURL}/api/v1/projects/delete-project/${id}`,
      {
        withCredentials: true,
      }
    );
...
export const updateProject = (id, newData) => async (dispatch) => {
  ...
  try {
    const response = await axios.put(
      `${APIBaseURL}/api/v1/projects/update-project/${id}`,
      newData,
      {
        withCredentials: true,
        headers: { "Content-Type": "multipart/form-data" },
